...
```text
VeriPix/
├── config.yaml                     # Global configuration (paths, hyperparameters, losses)
├── scripts/                        # Utility scripts
│   └── prepare_data.py             # Script to generate train/val/test splits
├── data/
│   ├── raw/                        # Original datasets (CASIA, CoMoFoD)
│   ├── processed/                  # Train/val/test splits (generated by script)
│   └── masks/                      # Ground truth masks
├── models/
│   ├── efficientnet_classifier.py  # EfficientNet-based binary classifier
│   └── unet_localizer.py           # U-Net based localization model
├── preprocessing/
│   └── ela.py                      # Error Level Analysis implementation
├── utils/
│   ├── dataset.py                  # Custom PyTorch dataset and dataloaders
│   └── advanced_augmentations.py   # Data augmentation pipelines
├── training/
│   ├── train_classifier.py         # Training loop for classifier
│   ├── train_localizer.py          # Training loop for localizer
│   └── losses.py                   # Loss functions (BCE, Dice, Focal, etc.)
├── evaluation/
│   └── evaluate.py                 # Metrics calculation (Acc, F1, AUC, IoU, Dice)
├── gui/
│   ├── gradio_app.py               # Full web interface (Classification + Localization)
│   └── gradio_app_classifier_only.py
├── checkpoints/                    # Saved model weights
├── logs/                           # TensorBoard logs
└── results/                        # Output plots (Confusion Matrix, ROC, etc.)
```
...
```
Input: ELA Images
Metrics: Accuracy (~90–95%), F1-Score (0.89–0.95), AUC-ROC (~0.97)
...
Metrics: IoU (0.2–0.7 depending on dataset difficulty), Dice Score
